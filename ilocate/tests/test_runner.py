...
class TestRunner(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        def date_data(dt):
            ...
            if dt.date() == date(2018, 11, 7):
                return [
                    {
                        'lon': '79.8887663', 'time_st': 'November 7, 2018, 05:12:00 AM', 'timestamp': 1541547720,
        ...
        def date_multi_data(dt1, dt2):
            return [
                ...
                },
                {
                    'lon': '79.8887663', 'time_st': 'November 5, 2018, 01:12:00 AM', 'timestamp': 1541360520,
                    'dist_from_last': 0, 'id': '1518', 'error': False, 'speed': 0, 'time_from_last': 0,
                    'device_type': '9', 'nic': None, 'charge_status': '1', 'number': '778352958',
                    'lat': '7.0598198', 'name': 'CAE-6745', 'update_type': '2', 'state': 'on'
                },
            ]

        cls.date_multi_data = date_multi_data

    def setUp(self):
        api = Mock()
        api.get_data = Mock(side_effect=self.date_data)
        api.get_data_range = Mock(side_effect=self.date_multi_data)
        persistence = Mock()
    ...
    @patch('ilocate.runner.dict_to_record_objects')
    def test_save_date_range(self, dict_to_rec):
        self.runner.retrieve_and_save_date_range(datetime(2018, 11, 5), datetime(2018, 11, 6))
        self.assertEqual(dict_to_rec.call_count, 3)
        self.assertEqual(self.runner.persistence.add_record.call_count, 1)
        self.assertGreaterEqual(self.runner.api.login.call_count, 1)
